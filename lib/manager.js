...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  function error (err, statusCode) {
    var code = (typeof statusCode == 'number' && statusCode >= 400) ? statusCode : 500;
    writeErr(code, 'handshake error');
    self.log.warn('handshake error with code: ' + code + ' error: ' + err);
  };
  ...
  this.authorize(handshakeData, function (err, statusCode, authorized, newData) {
...
Manager.prototype.authorize = function (data, fn) {
  if (this.get('authorization')) {
    ...
    this.get('authorization').call(this, data, function (err, statusCode, authorized) {
      self.log.debug('client ' + authorized ? 'authorized' : 'unauthorized');
      fn(err, statusCode, authorized);
    });
  } else {
